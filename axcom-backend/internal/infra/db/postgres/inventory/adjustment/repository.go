...
func (r *PostgresRepository) GetStock(ctx context.Context, variantID string, locationID string) (*domain.StockItem, error) {
	ctx, span := otel.Tracer("postgres").Start(ctx, "PostgresInventoryAdjustmentRepository.GetStock")
	defer span.End()
...
func (r *PostgresRepository) SaveStock(ctx context.Context, stock *domain.StockItem) error {
	ctx, span := otel.Tracer("postgres").Start(ctx, "PostgresInventoryAdjustmentRepository.SaveStock")
	defer span.End()
...
func (r *PostgresRepository) AdjustQuantity(ctx context.Context, variantID, locationID string, delta int) error {
	ctx, span := otel.Tracer("postgres").Start(ctx, "PostgresInventoryAdjustmentRepository.AdjustQuantity")
	defer span.End()
